...
/*   Updated: 2025/04/09 18:29:33 by lilefebv         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_glyph_outline(t_img *img, t_ttf *ttf, t_uchar c, t_point2 pos)
{
	...
	while (i < ttf->glyph256[c].bezier_amount)
	{
		...
		{
			t_point p1;
			p1.x = (ttf->glyph256[c].bezier_lines[i].p1.x + ttf->glyph256[c].advance_width) * scale + pos.x;
			p1.y = ttf->glyph256[c].bezier_lines[i].p1.y * -scale + pos.y;
			t_point p2;
			p2.x = (ttf->glyph256[c].bezier_lines[i].p2.x + ttf->glyph256[c].advance_width) * scale + pos.x;
		...
		else
		{
			...
			{
				t_point p1;
				p1.x = (points[y].x + ttf->glyph256[c].advance_width) * scale + pos.x;
				p1.y = points[y].y * -scale + pos.y;
				t_point p2;
				p2.x = (points[y + 1].x + ttf->glyph256[c].advance_width) * scale + pos.x;
