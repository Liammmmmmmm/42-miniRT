...
/*   Updated: 2025/06/20 14:35:42 by lilefebv         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

static int	parse_cyl2(t_scene *scene, char **parts, t_cylinder *cyl,
	int nb_parts)
{
...
int	parse_cylinder(t_scene *scene, char *line)
{
	...
	if (!parse_vector_normalized(parts[2], &cyl->orientation))
		return (invalid_struct_error(CYLINDER, parts));
	if (parse_cyl2(scene, parts, cyl, nb_parts) == 0)
		return (0);
	free(parts);
	return (1);
}

static int	more_mat_props(char **parts, t_mat *mat, t_scene *scene,
	int nb_parts)
{
	if (parse_color_or_tex(parts[9], &mat->emission_color_tmp,
			&mat->emission_color_tex, scene) == 0)
		return (0);
	mat->scale = 1;
	if (nb_parts >= 11 && is_valid_double_el_no_bordered(parts[10],
			&mat->scale) == 0)
	...
	if (nb_parts >= 13 && is_valid_double_el_no_bordered(parts[12],
			&mat->normal_intensity) == 0)
	...
	int nb_parts)
{
	...
	if (parse_double_b_or_tex(parts[6], &mat->transmission_value,
			&mat->transmission_tex, scene) == 0)
	...
	if (parse_double_or_tex(parts[8], &mat->emission_strength,
			&mat->emission_strength_tex, scene) == 0)
		return (material_item_error(6, mat->name));
	if (more_mat_props(parts, mat, scene, nb_parts) == 0)
		return (0);
	return (1);
}
