...
/*   Updated: 2025/04/27 16:03:15 by madelvin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline t_ray_data	refracted_value(t_mat_manager *mat_man, double eta)
{
	t_ray_data	ray_data;
	t_vec3		direction;

	direction = refracted_v(mat_man->ray_in.dir, \
		mat_man->hit_record.normal, eta);
	...
	ray_data = ray_color(mat_man->minirt, mat_man->ray_in, \
		mat_man->depth - 1, NULL);
	return ((t_ray_data){color_multiply(mat_man->hit_record.color, \
...
static inline t_ray_data	refracted_ray(t_mat_manager *mat_man)
{
	...
	cos_theta = fmin(-vec3_dot(mat_man->ray_in.dir, \
		mat_man->hit_record.normal), 1.0);
	sin_theta = sqrt(1.0 - cos_theta * cos_theta);
	if (eta * sin_theta > 1.0)
		return (metallic_material(mat_man));
	else
		return (refracted_value(mat_man, eta));
}
