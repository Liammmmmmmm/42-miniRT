...
/*   Updated: 2025/06/30 15:51:50 by madelvin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char    init_photon_map(t_kd_tree *photon_map, t_minirt *minirt, int photons_per_light)
{
    ...
    // CORRECTION : On initialise le vecteur pour stocker la structure t_photon complète.
    ...
    printf("%zu photons stored.\n", count);
    printf("K-D Tree build...\n");
    if (kd_tree_build(photon_map, &all_photon))
    {
...
int	caustic_manager(t_minirt *minirt, int photons_to_shoot)
{
	...
	if (minirt->scene.photon_map.root)
	{
		...
		printf("ptn: %d\n", minirt->scene.photon_map.photons[0].i);
	}
	else
	{
		kd_tree_destroy(&minirt->scene.photon_map);
