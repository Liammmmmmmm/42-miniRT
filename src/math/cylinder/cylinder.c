...
/*   Updated: 2025/04/20 17:01:55 by madelvin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_hit_record *rec, t_quadratic *q)
{
	...
	p = ray_at(*r, q->t_hit);
	proj = vec3_dot(vec3_subtract(p, cyl->position), axis);
	if (proj < 0.0 || proj > cyl->height)
		return (0);
	outward = vec3_subtract(vec3_subtract(p, cyl->position), \
...
char	cylinder_cap_bottom(t_ray *r, t_cylinder *cyl, t_interval interval, t_hit_record *rec)
{
	...
	if (fabs(denom) < 1e-6)
		return (0);
	t = -vec3_dot(vec3_subtract(r->orig, cyl->position), cyl->orientation) / denom;
	if (t < interval.min || t > interval.max)
		return (0);
	hit_point = ray_at(*r, t);
	if (vec3_length(vec3_subtract(hit_point, cyl->position)) > cyl->diameter / 2.0)
		return (0);
	rec->t = t;
	rec->point = hit_point;
...
char	cylinder_cap_top(t_ray *r, t_cylinder *cyl, t_interval interval, t_hit_record *rec)
{
	const t_vec3	plane_origin = vec3_add(cyl->position, \
						vec3_multiply_scalar(cyl->orientation, cyl->height));
	double			t;
	const double	denom = vec3_dot(r->dir, cyl->orientation);
	t_vec3			hit_point;
	...
	if (fabs(denom) < 1e-6)
		return (0);
	t = -vec3_dot(vec3_subtract(r->orig, plane_origin), cyl->orientation) / denom;
	if (t < interval.min || t > interval.max)
		return (0);
	hit_point = ray_at(*r, t);
	if (vec3_length(vec3_subtract(hit_point, plane_origin)) > cyl->diameter / 2.0)
		return (0);
	rec->t = t;
	rec->point = hit_point;
...
char	hit_cylinder(t_cylinder *cyl, t_ray *r, t_interval interval, t_hit_record *rec)
{
	t_quadratic	q;
	int			hit_cap_bottom;
	int			hit_cap_top;
	...
	if (!init_cylinder_quadratic(&q, cyl, r))
		return (0);
	if (!valid_t(&q, interval))
		return (0);
