...
/*   Updated: 2025/06/23 17:54:35 by madelvin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	apply_transformation_tirangle(t_triangle *tri,
	t_vertex_indexed v[3], t_custom_object *obj, size_t i)
{
...
static char	init_malloc_obj_finilizer(t_custom_object *obj, t_mesh *mesh)
{
...
int	flatten_mesh_to_object(t_custom_object *obj, t_mesh *mesh)
{
	...
	if (init_malloc_obj_finilizer(obj, mesh))
		return (1);
	i = 0;
	while (i < obj->triangle_count)
	{
		...
		apply_transformation_tirangle(&obj->triangles[i], v, obj, i);
		i++;
	}
	obj->name = mesh->name;
