...
/*   Updated: 2025/06/10 15:15:44 by lilefebv         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_list_objects(t_minirt *minirt)
{
	...
	while (++i < minirt->scene.el_amount)
		draw_object(&minirt->mlx.img_controls, &minirt->controls.font[0],
			minirt, i);
}
...
void	get_clicked_object(t_minirt *minirt, int mouse_y)
{
	...
	while (++i < minirt->scene.el_amount)
	{
		...
		{
			minirt->controls.ui_infos.selected_object
				= &minirt->scene.elements[i];
			set_selected_object(minirt,
				minirt->controls.ui_infos.selected_object);
