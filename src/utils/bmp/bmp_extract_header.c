...
/*   Updated: 2025/04/10 19:14:34 by madelvin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	extract_raw_pixels(t_bin *bin, size_t *i, t_bmp *bmp)
{
	...
	y = 0;
	x = 0;
	bmp->pixel_data = malloc(bmp->info.with * bmp->info.height * \
								sizeof(uint32_t));
	if (bmp->pixel_data == NULL)
		return (error_and_return("error: unable to access to memory"));
	while (y < bmp->info.height)
	{
		x = 0;
		while (x < bmp->info.with)
		{
			if (extract_pixel(bin, i, bmp, \
				&bmp->pixel_data[y * bmp->info.with + x]) == -1)
			{
				free(bmp->pixel_data);
				return (1);
			}
