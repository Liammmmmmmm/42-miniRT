...
/*   Updated: 2025/04/03 14:55:15 by lilefebv         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
uint32_t	get_glyph_offset(t_bin *bin, t_ttf *ttf, uint32_t glyph_index)
{
	...
	if (read_loca_type(bin, ttf))
		read_uint32(bin, ttf->r_data.loca_offset + glyph_index * sizeof(uint32_t), &offset);
	else
	{
		off16 = 0;
		read_uint16(bin, ttf->r_data.loca_offset + glyph_index * sizeof(uint16_t), &off16);
