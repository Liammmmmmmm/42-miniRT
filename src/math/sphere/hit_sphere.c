	...
	t_interval interval, t_hit_record *rec)
{
	...
	d = b * b - a * ((oc.x * oc.x + oc.y * oc.y + oc.z * oc.z) - \
		sphere.sqrt_diameter);
	if (d < 0)
		return (0);
	sqrtd = sqrt(d);
	root = (-b - sqrtd) / a;
	...
	oc = vec3_divide_scalar((t_vec3){rec->point.x - sphere.position.x, \
			rec->point.y - sphere.position.y, rec->point.z - \
			sphere.position.z}, sphere.diameter);
